...
<a class="btnExport" href='#'></i>导出Excel</a>
...
<script>
    ...
    $(function () {
        ...
        $('.btnExport').click(function () {
            var missionId = $("#filter_missionId").val();
            setTimeout(function () {
                download("/execl");
            }, 1000);
        });
...
public class ExeclController {
    ...
    @GetMapping("/execl")
    public void execl(HttpServletRequest request, HttpServletResponse response, ModelMap model){

        String[] properties = {"nickName"};
        String[] titles = {"昵称"};
        Integer[] sizes =  new Integer[]{6000};
        ExcelView ev = new ExcelView("任务的提交数据.xls", "任务的提交数据", properties, titles
                ,sizes, null, getList(), null);
    ...
    public List<Execl> getList(){
        return Arrays.asList(new Execl("hgx"),new Execl("hgx2"));
    }
