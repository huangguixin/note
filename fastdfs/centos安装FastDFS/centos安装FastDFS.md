...
```shell
yum -y install perl net-tools wget
```

如果一步一步安装觉得麻烦，下面这条命令一次性安装
...
```shell
base_path=/root/fastdfs/tracker # tracker的数据和日志存放目录
```
...
```shell
sudo mkdir -p /root/fastdfs/tracker
```
...
```shell
base_path=/root/fastdfs/storage # storage的数据和日志存放目录
store_path0=/root/fastdfs/storage # storage的上传文件存放路径
...
```shell
sudo mkdir -p /root/fastdfs/storage
```
...
- 修改一下配置：
  ...
  store_path0=/root/fastdfs/storage        		# 文件存储路径
  ```
...
- 配置
  ...
  ```shell
  sudo ./configure --prefix=/opt/nginx --sbin-path=/usr/bin/nginx --add-module=/root/fastdfs-nginx-module/src
  ```
